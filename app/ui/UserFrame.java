...
public class UserFrame extends JFrame {
  ...
  private void handleReserve() {
    ...
    try {
      ReservationFactory factory = new LectureReservationFactory(manager);
  ...
  private void handleRent() {
    ...
    ReservationFactory factory = new ItemReservationFactory(manager);
  ...
  private void ensureSampleRentals() {
    if (manager.getRentables().size() == 0) {
      manager.addRentableResource(new SimpleItem("노트북", 100000));
      manager.addRentableResource(new SimpleItem("우산", 10000));
    }
  }
